...
export default function UploadForm() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      `}</style>
      <div className="bg-white rounded-xl border border-slate-200 p-10 lg:p-12 shadow-sm hover:shadow-md transition">
        {/* Header */}
        <div className="mb-10">
          <h2 className="text-3xl font-bold mb-3" style={{color: '#362c5d'}}>Upload Your Lecture</h2>
          <p className="text-slate-600">
            Upload a video and optional slides to generate comprehensive study materials
          </p>
        </div>
        ...
        <div className="space-y-8">
          {/* Lecture Title Input */}
          <div>
            <label htmlFor="lecture-title" className="block text-sm font-semibold mb-3" style={{color: '#362c5d'}}>
          ...
          {/* Video File Upload */}
          <div>
            <label className="block text-sm font-semibold mb-3" style={{color: '#362c5d'}}>
              Video File <span style={{color: '#c84449'}}>*</span>
              <span className="text-slate-500 font-normal ml-2">(MP4, MOV, AVI, WebM)</span>
            </label>
            <div
            ...
            >
              ...
              >
                <div className="flex justify-center mb-4">
                  {video ? (
                    <div className="p-3 rounded-lg" style={{backgroundColor: 'rgba(200, 68, 73, 0.1)'}}>
                      <CheckCircle2 className="w-8 h-8" style={{color: '#c84449'}} />
                    </div>
                  ) : (
                    <div className="p-3 rounded-lg" style={{backgroundColor: 'rgba(54, 44, 93, 0.1)'}}>
                      <Upload className="w-8 h-8" style={{color: '#362c5d'}} />
                    </div>
                  )}
                </div>
                ...
                <p className="text-slate-600 text-sm">
                  {video ? `${video.name} • ${formatFileSize(video.size)}` : 'Maximum size: 500MB'}
                </p>
              </label>
            </div>
          </div>
              ...
              >
                <div className="flex justify-center mb-4">
                  {slides ? (
                    ...
                    <div className="p-3 rounded-lg" style={{backgroundColor: 'rgba(54, 44, 93, 0.1)'}}>
                      <FileText className="w-8 h-8" style={{color: '#362c5d'}} />
                    </div>
                  )}
                </div>
                <p className="font-semibold mb-1\" style={{color: '#362c5d'}}>
                  {slides ? 'Slides selected' : 'Click or drag slides to upload'}
                </p>
                <p className="text-slate-600 text-sm\">
                  {slides ? `${slides.name} • ${formatFileSize(slides.size)}` : 'Optional - helps with context'}
                </p>
              </label>
            </div>
          </div>
          ...
          <button
            ...
            style={{
              backgroundColor: (!video || !title || isLoading) ? '#cbd5e1' : '#362c5d',
